<template>
  <v-dialog max-width="500px" v-model="open" >
    <v-card class="addDialog">
      <v-card-title>Add Travel</v-card-title>
      <v-form>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="saveTravel(code=code,code_route=code_route,start_time=start_time,end_time=end_time,rut_driver=rut_driver,code_bus=code_bus)" color="green">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods:{
    async saveTravel(code,code_route,start_time,end_time,rut_driver,code_bus){
        ...
        await fetch(
        `http://127.0.0.1:8000/back/travel?code=${code}&route=${code_route}&start_time=${start_time}&end_time=${end_time}&rut_driver=${rut_driver},&code_bus=${code_bus}`,{
            method:"POST",
            mode:'cors',
            headers:{
