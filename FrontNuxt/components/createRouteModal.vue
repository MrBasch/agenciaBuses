<template>
  <v-dialog max-width="500px" v-model="open" >
    <v-card class="addDialog">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="saveRoute(code=code,name=name,list_stations=stations_selected,state=state)" color="green">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods:{
    async saveRoute(name,code,list_stations,state){
        ...
        console.log("code = ", code ,"name =", name, "list_stations = ", list_stations, "state = ", state)
        await fetch(
