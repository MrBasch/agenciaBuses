  ...
  <v-dialog max-width="500px" v-model="edit" >
    <v-card class="addDialog">
      ...
      <v-form>
        ...
        <v-text-field v-model="new_code" label="Enter station new_code"></v-text-field>
        <v-select v-model="city" :items="cities" label="Select a city"></v-select>
        
      </v-form>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="updateStation(name=name,code=code,city_id=city.id)" color="#417D7A">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
