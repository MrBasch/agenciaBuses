<template>
  <v-dialog max-width="500px" v-model="edit" >
    <v-card class="addDialog">
      <v-card-title>Update or create bus</v-card-title>
      <v-form>
        <v-text-field v-model="code" label="Enter bus code"></v-text-field>
        <v-text-field v-model="new_code" label="Enter the new bus code"></v-text-field>
        <v-select v-model="state" :items="status" label="Select A Status"></v-select>
      </v-form>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="updateBus(code=code,status=state, new_code=new_code)" color="green">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
