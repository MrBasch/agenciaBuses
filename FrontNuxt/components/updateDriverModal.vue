  ...
  <v-dialog max-width="500px" v-model="edit" >
    <v-card class="addDialog">
      ...
      <v-form>
        ...
        <v-text-field v-model="new_rut" label="Enter new driver rut"></v-text-field>
        <v-select v-model="state" :items="status" label="Select a state"></v-select>
      </v-form>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="updateDriver(name=name,rut=rut,new_rut=new_rut,status=state,)" color="#417D7A">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods:{
    async updateDriver(name,rut,new_rut,status){
        console.log("status", status,"name", name,"rut", rut,"new_rut", new_rut)
        await fetch(
