<template>
  <v-dialog max-width="500px" v-model="edit" >
    <v-card class="addDialog">
      <v-card-title>update or create Route</v-card-title>
      <v-form>
        ...
        <v-select multiple v-model="stations_selected" :items="stations" label="Select stations"></v-select>
        <v-select v-model="state" :items="status" label="Select A Status"></v-select>
      </v-form>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="updateRoute(name=name,code=code,new_code=new_code,list_stations=stations_selected,state=state)" color="green">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods:{
    async updateRoute(name,code,new_code,list_stations,state){
        console.log("code = ", code ,"new_code = ", new_code ,"name =", name, "list_stations = ", list_stations, "state = ", state)
        await fetch(
      ...
      async getStations() {
        await fetch(
        'http://127.0.0.1:8000/back/station',{
            method:"GET",
            mode:'cors',
            headers:{
              "Authorization":`Token 766c1cfc137888181d2c6f8b0e14a7dd2dc1f8ea`
