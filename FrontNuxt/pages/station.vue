            ...
            <li v-for="(station, index) in stations" :key="station.rut">
                <v-card height="5rem" class="item_card">
                    ...
                    <div class="buttons">
                        ...
                        <button class="delete" @click="deleteStation(code=station.code,index)"><v-icon>mdi-delete</v-icon></button>
                    </div>
                </v-card>
            </li>
        </ul>
...
export default {
    components:{
        createStationModal,updateStationModal
        },
    data() {
      return {
        open:false,
    ...
    methods:{
        async deleteStation(index,code){
            ...
            await fetch(
            `http://127.0.0.1:8000/back/station?code=${code}`,{
                method:"DELETE",
                mode:'cors',
                headers:{
