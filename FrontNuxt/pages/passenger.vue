            ...
            <li v-for="(passenger, index) in passengers" :key="passenger.rut">
                <v-card height="5rem" class="item_card">
                    ...
                    <div class="buttons">
                        ...
                        <button class="delete" @click="deletePassenger(index=index)"><v-icon>mdi-delete</v-icon></button>
                    </div>
                </v-card>
            </li>
        </ul>
...
export default {
    components:{
        createPassengerModal,updatePassengerModal
        },
    data() {
      return {
        open:false,
    ...
    methods:{
        async deletePassenger(index,){
            ...
            await fetch(
            `http://127.0.0.1:8000/back/passenger?id=${index}`,{
                method:"DELETE",
                mode:'cors',
                headers:{
        ...
        async getPassengers() {
            await fetch(
            'http://127.0.0.1:8000/back/passenger',{
                method:"GET",
                mode:'cors',
                headers:{
                "Authorization":`Token 766c1cfc137888181d2c6f8b0e14a7dd2dc1f8ea`
